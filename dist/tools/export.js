...
function buildMarkdown(data) {
    ...
    if (v.category)
        lines.push(`| **Category** | ${escMd(v.category)} |`);
    lines.push('');
    // ── Topics ──────────────────────────────────────────────────────────────────
    if (s?.topics?.length) {
        lines.push(`## Topics`);
        lines.push('');
        lines.push(s.topics.map((t) => `\`${t}\``).join('  '));
        lines.push('');
    }
    // ── Description snippet ─────────────────────────────────────────────────────
    if (v.description) {
    ...
    if (v.tags?.length) {
        lines.push(`## Tags`);
        lines.push('');
        lines.push(v.tags.slice(0, 30).map((t) => `\`${t}\``).join(' '));
        lines.push('');
    }
    // ── Footer ───────────────────────────────────────────────────────────────────
    lines.push('---');
...
export async function handleExport(args) {
    ...
    if (args.includeTranscript)
        tasks.push(getTranscript(args.videoId, args.language));
    if (args.includeComments)
        tasks.push(handleComments({ videoId: args.videoId, limit: 20 }));
    ...
    if (args.includeTranscript && results[2]?.status === 'fulfilled') {
        ...
        if (tr && !('error' in tr)) {
            condensedTranscript = cleanTranscriptText(tr.segments?.map((s) => s.text).join(' ') ?? '');
        }
    }
    // Parse comments
    let commentList = [];
    ...
    if (!videoInfo) {
        return {
            content: [{ type: 'text', text: JSON.stringify({ error: `Could not fetch video info for ${args.videoId}` }) }],
            isError: true,
        };
    }
    const data = {
