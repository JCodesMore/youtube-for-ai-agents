...
function scoreSegment(text: string): number {
  ...
  // Signal boosters
  if (/\d/.test(t)) score += 0.5;                          // contains numbers
  if (/\bbecause\b|\btherefore\b|\bso\b|\bthus\b/.test(t)) score += 0.4;  // causal
  if (/\bimportant\b|\bkey\b|\bmain\b|\bcritical\b/.test(t)) score += 0.4; // emphasis
  if (/\bfirst\b|\bsecond\b|\bfinally\b|\bmost\b/.test(t)) score += 0.3;  // enumeration

  // Filler penalties
  if (/^(um|uh|like|you know|so|and|but)\b/.test(t)) score -= 0.5;
  if (/\bclick\b|\bsubscribe\b|\blike\s+this\b/.test(t)) score -= 0.6;    // channel-plug
...
interface SectionResult {
  ...
  startTime: number;
  endTime: number;
  keySentences: Array<{ text: string; timestamp?: number }>;
}

export async function handleSummarize(args: SummarizeArgs) {
  ...
  // Build sections from chapters or fallback to uniform 3-minute chunks
  type Section = { title: string; startTime: number; endTime: number };
  ...
  const extracted: SectionResult[] = sections.map(sec => {
    const segs = transcript.segments.filter(
      s => s.offset >= sec.startTime && s.offset < sec.endTime,
    );
