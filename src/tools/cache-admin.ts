...
const NAMED_CACHES: Record<string, typeof videoInfoCache> = {
...
export const cacheAdminInputSchema = {
  action: z.enum(['stats', 'invalidate', 'warm', 'events']).describe(
    '"stats" — show sizes and bus event counts. ' +
...
export async function handleCacheAdmin(args: CacheAdminArgs) {
  switch (args.action) {
    ...
    case 'invalidate': {
      if (!args.videoIds?.length) {
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ error: 'Provide videoIds to invalidate.' }) }],
          isError: true,
        };
      }
    ...
    case 'warm': {
      if (!args.videoIds?.length) {
        return {
          content: [{ type: 'text' as const, text: JSON.stringify({ error: 'Provide videoIds to warm.' }) }],
          isError: true,
        };
      }
    ...
    case 'events': {
      ...
      const topic = args.eventTopic as any;
