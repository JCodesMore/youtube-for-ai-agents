...
import type { ChannelInfo, ChannelVideo } from '../lib/innertube.js';
...
interface CompareResponse {
  ...
  ranking: {
    bySubscribers: string[];
    byAvgViewsPerVideo: string[];
    byTotalViews: string[];
    byUploadCadence: string[];
  };
...
function computeCadenceDays(videos: ChannelVideo[]): number | null {
  ...
  try {
    const [info, videosResult] = await Promise.all([
      ...
      recentVideoCount > 0
        ...
        : Promise.resolve({ channelId: '', channelName: '', videoCount: 0, videos: [] as ChannelVideo[] }),
    ]);
...
function rankBy(
  channels: ChannelComparison[],
  selector: (c: ChannelComparison) => number,
  desc = true,
): string[] {
  return [...channels]
    .filter((c) => !c.error)
...
export async function handleChannelCompare(args: {
  ...
  const results = await Promise.all(
    args.channels.map((c) => fetchOne(c, recentVideoCount, includeRecentTitles)),
  );
