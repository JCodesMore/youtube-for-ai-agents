...
import { z } from 'zod';
import { getTranscript, type TranscriptSegment } from '../lib/transcript.js';
...
function buildChunks(segments: TranscriptSegment[], chunkChars: number): TranscriptSegment[][] {
...
async function translateChunk(
  ...
  try {
    ...
    if (typeof json.translatedText !== 'string') throw new Error('missing translatedText in response');
...
function distributeTranslatedText(
...
export async function handleTranscriptTranslate(args: TranslateArgs) {
  ...
  if (args.startTime !== undefined) segments = segments.filter(s => s.offset >= args.startTime!);
  if (args.endTime !== undefined) segments = segments.filter(s => s.offset < args.endTime!);
  if (args.maxSegments !== undefined) segments = segments.slice(0, args.maxSegments);

  if (segments.length === 0) {
    return {
      content: [{ type: 'text' as const, text: JSON.stringify({ error: 'No segments to translate after filtering.' }) }],
      isError: true,
    };
  }
  ...
  const results = await pooledMap(chunkTexts, config.translate.concurrency, (text) =>
    translateChunk(text, engineUrl, source, target, apiKey || undefined, config.translate.timeoutMs)
  );
  ...
  if (format === 'segments' || format === 'both') response.segments = translatedSegments;
  if (format === 'text' || format === 'both') response.fullText = translatedFullText;
