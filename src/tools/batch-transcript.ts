...
export async function handleBatchTranscript(args: BatchTranscriptArgs) {
  const format = args.format ?? 'both';

  // Fetch all transcripts + video info concurrently
  const tasks = args.videoIds.map(async (videoId): Promise<VideoResult> => {
    ...
    if (format === 'segments' || format === 'both') result.segments = segments;
    if (format === 'text' || format === 'both') result.fullText = fullText;
