...
function union(sets: Set<string>[]): Set<string> {
  ...
  for (const s of sets) for (const w of s) out.add(w);
  return out;
}
...
function extractTitle(texts: string[]): string {
  ...
  for (const t of texts) {
    ...
    if (/\d/.test(trimmed)) score += 0.3;
    if (/\bbecause\b|\btherefore\b|\bso\b/.test(trimmed.toLowerCase())) score += 0.3;
    if (/^(um|uh|like|you know|so,|and,)/i.test(trimmed)) score -= 0.8;
    if (/subscribe|click|like this/i.test(trimmed)) score -= 1;
    if (score > bestScore) { bestScore = score; bestText = trimmed; }
  }

  if (!bestText) return texts[0]?.trim().slice(0, 60) ?? 'Chapter';
...
export async function handleChaptersEdit(args: ChaptersEditArgs) {
  ...
  if (transcriptResult.status === 'rejected') {
    return { content: [{ type: 'text' as const, text: JSON.stringify({ error: String(transcriptResult.reason) }) }], isError: true };
  }
  const transcript = transcriptResult.value;
  if ('error' in transcript) {
    return { content: [{ type: 'text' as const, text: JSON.stringify(transcript) }], isError: true };
  }
  ...
  const scores: number[] = new Array(segments.length).fill(0);
  ...
  for (const { i } of indexed) {
    if (breakPoints.length >= maxChapters - 1) break;
