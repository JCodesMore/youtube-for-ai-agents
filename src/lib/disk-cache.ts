...
function getCacheDir(): string {
  ...
  if (pluginData) return join(pluginData, 'cache', 'transcripts');
...
interface DiskEntry<T> {
  ...
  version: number;     // bump to invalidate old format
}
...
export class DiskCache<V> {
  ...
  get(key: string): V | undefined {
    ...
    try {
      ...
      const entry: DiskEntry<V> = JSON.parse(raw);
      if (entry.version !== CURRENT_VERSION) { unlinkSync(fp); return undefined; }
      if (Date.now() > entry.expiresAt) { unlinkSync(fp); return undefined; }
      return entry.value;
    } catch {
      return undefined;
    }
  ...
  invalidate(key: string): void {
    try { unlinkSync(this.path(key)); } catch { /* ok */ }
  }
}

// 7-day transcript cache — transcripts almost never change
export const transcriptDiskCache = new DiskCache<unknown>(24 * 7);
